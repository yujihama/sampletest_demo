...
import { GraphExecutionPanel } from "./components/GraphExecutionPanel";
...
export function AgentInbox() {
  ...
  const MainViewComponent = isStateViewOpen 
    ? <ThreadView threadId={selectedThreadIdParam} /> 
    : <AgentInboxView saveScrollPosition={saveScrollPosition} containerRef={containerRef} />;

  return (
    <div className="flex flex-row w-full h-full">
      <div className="w-[30%] h-full overflow-y-auto border-r flex-shrink-0 min-w-0">
        {MainViewComponent}
      </div>

      <div className="w-[70%] h-full overflow-y-auto overflow-x-auto border-l flex-shrink-0 min-w-0">
        <GraphExecutionPanel />
      </div>
    </div>
  );
